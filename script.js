...
// ==========================================
// CONFIGURACIÓN DEL SEGUNDO GRÁFICO: BUBBLE
// ==========================================

// 1. Datos base extraídos del primer gráfico
const datosBaseParaBurbujas = [
    { name: "BACHILLERATO EN ARTES, ARQUITECTURA Y DISEÑO EN LA UNIVERSIDAD FINIS TERRAE", x: 5, y: 7200000 },
    { name: "BACHILLERATO EN DISEÑO EN LA UNIVERSIDAD DEL DESARROLLO", x: 6, y: 10364360 },
    { name: "DISEÑO EN LA UNIVERSIDAD AUSTRAL DE CHILE", x: 6, y: 5887000 },
    { name: "DISEÑO EN LA UNIVERSIDAD CATÓLICA DE TEMUCO", x: 5, y: 5276000 },
...
// 2. Clasificación de datos en categorías mediante filtros de texto
const digitalVideojuegos = datosBaseParaBurbujas
    .filter(item => item.name.includes("VIDEOJUEGOS") || item.name.includes("DIGITAL") || item.name.includes("ANIMACIÓN") || item.name.includes("INMERSIVAS") || item.name.includes("VIRTUAL"))
    .map(item => ({ x: item.x, y: item.y, r: 12, name: item.name }));

const industrialProductos = datosBaseParaBurbujas
    .filter(item => item.name.includes("INDUSTRIAL") || item.name.includes("PRODUCTOS") || item.name.includes("FABRICACION"))
    .map(item => ({ x: item.x, y: item.y, r: 12, name: item.name }));

const graficoVisual = datosBaseParaBurbujas
    .filter(item => (item.name.includes("GRÁFICO") || item.name.includes("VISUAL") || item.name.includes("MULTIMEDIA")) && !digitalVideojuegos.some(d => d.name === item.name))
    .map(item => ({ x: item.x, y: item.y, r: 12, name: item.name }));

const disenoGeneralOtros = datosBaseParaBurbujas
    .filter(item => !digitalVideojuegos.some(d => d.name === item.name) && !industrialProductos.some(i => i.name === item.name) && !graficoVisual.some(g => g.name === item.name))
    .map(item => ({ x: item.x, y: item.y, r: 12, name: item.name }));

// 3. Inicialización del gráfico Bubble apuntando al ID "burbuja"
new Chart(document.getElementById("burbuja"), {
    type: 'bubble',
    data: {
        datasets: [
            {
                label: 'Digital y Videojuegos',
                data: digitalVideojuegos,
                backgroundColor: 'rgba(232, 87, 26, 0.75)', 
                borderColor: 'rgb(232, 87, 26)',
                borderWidth: 1
            },
            {
                label: 'Industrial y Productos',
                data: industrialProductos,
                backgroundColor: 'rgba(70, 130, 180, 0.75)', 
                borderColor: 'rgb(70, 130, 180)',
                borderWidth: 1
            },
            {
                label: 'Gráfico y Visual',
                data: graficoVisual,
            ...
            {
                label: 'General y Otras Ramas',
                data: disenoGeneralOtros,
    ...
    options: {
        ...
        plugins: {
            legend: {
                position: 'top',
                labels: {
                    font: { family: "'Georama', sans-serif", weight: '700' }
                }
            },
            title: {
                display: true,
                text: 'Tipo de diseño Vs. Costo del Arancel',
            ...
            tooltip: {
                ...
                callbacks: {
                    label: function (context) {
                        ...
                        return [
                            point.name,
                            "Acreditación Institucional: " + point.x + " años",
